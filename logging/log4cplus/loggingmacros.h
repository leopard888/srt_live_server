...
#define LOG4CPLUS_RESTORE_DOWHILE_WARNING() __pragma(warning(pop))
...
#define LOG4CPLUS_DOWHILE_NOTHING()            \
    LOG4CPLUS_SUPPRESS_DOWHILE_WARNING() do {} \
    while (0) LOG4CPLUS_RESTORE_DOWHILE_WARNING()
...
inline Logger macros_get_logger(Logger const &logger) { return logger; }

inline Logger const &macros_get_logger(Logger &logger) { return logger; }

#if defined(LOG4CPLUS_HAVE_RVALUE_REFS)
inline Logger macros_get_logger(Logger &&logger) { return std::move(logger); }

#endif

inline Logger macros_get_logger(tstring const &logger) {
    return Logger::getInstance(logger);
}

inline Logger macros_get_logger(tchar const *logger) {
    return Logger::getInstance(logger);
}
...
#else
#define LOG4CPLUS_MACRO_INSTANTIATE_OSTRINGSTREAM(var) \
    log4cplus::tostringstream &var = log4cplus::detail::get_macro_body_oss()
    ...
    LOG4CPLUS_SUPPRESS_DOWHILE_WARNING() do {                            \
        ...
        if (LOG4CPLUS_MACRO_LOGLEVEL_PRED(                               \
            ...
            log4cplus::detail::macro_forced_log(                         \
                _l, log4cplus::logLevel, _log4cplus_buf.str(), __FILE__, \
                __LINE__, LOG4CPLUS_MACRO_FUNCTION());                   \
        }                                                                \
    }                                                                    \
    while (0) LOG4CPLUS_RESTORE_DOWHILE_WARNING()
...
#define LOG4CPLUS_MACRO_STR_BODY(logger, logEvent, logLevel)                  \
    LOG4CPLUS_SUPPRESS_DOWHILE_WARNING() do {                                 \
        ...
        if (LOG4CPLUS_MACRO_LOGLEVEL_PRED(                                    \
                _l.isEnabledFor(log4cplus::logLevel), logLevel)) {            \
            log4cplus::detail::macro_forced_log(_l, log4cplus::logLevel,      \
...
#define LOG4CPLUS_TRACE_METHOD(logger, logEvent)                               \
    log4cplus::TraceLogger _log4cplus_trace_logger(logger, logEvent, __FILE__, \
                                                   __LINE__);
